---
title: "Embedded Lecture-2: Startup File"
...
-   Code written in startup file runs before main(). So, you can say startup files calls main().
...
-   Startup code is responsible of .data, .bss sectuon initilization in main.

### Start up file code writing
...
### References
-   Vector table reference in MCU datasheet.
-   15 system execptions and 82 IRQs.
-   Create an array to hold MSP and handler addresses.
    collection of address uint32_t vectors[] = {store MSP and address of various handlers here};
-   Instruct the compiler not to include the above array in .data section but in a different user defined section.

### Need to know the boundart information to copy the .data 
-   boundaries information has to be exported from linker script
...
/* We have got 97 execptions (15 systesm execptions + 82 interrupts).So would you write handlers
 * for all the execptions?
 * Not required. Lets create a single default handler for all the execption and allow programmer to
 * implement required handlers as per application requirements.
 */

/* Function attribute : weak and alias
 * Weak : Lets programmer to override already define weak function (dummy) with the same function name
 * Alias : Lets programmer to give alias name for a function
 */

void Reset_Handler(void);
/* Here Default_Handler is a alias function name for NMI_Handler
 * In vector table array, address of default handler function will be stored.
 * So when NMI execption triggers, default handler will be executed.
 */

/* Now, we are making this weak. This is because, we are allowing programmer to override this function
 * with same function name in main application. There programmer can implement real implementaion of
 * handling that execption.
 */
